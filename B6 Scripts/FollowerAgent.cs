...
public class FollowerAgent : MonoBehaviour
{
    ...
    private void Update()
    {
        if (path.Count > 0 && Vector3.Distance(transform.position, path[0]) < 2.1f)
        {
            path.RemoveAt(0);
        }
    }
    ...
    private Vector3 CalculateAgentForce()
    {
        ...
        foreach (GameObject neighbor in perceivedNeighbors)
        {
            ...
            float oRadius = 0f;
            Vector3 oVelocity = Vector3.zero;

            if(neighbor.GetComponent<FollowerAgent>()){
                FollowerAgent oagent = neighbor.GetComponent<FollowerAgent>();
                oRadius = oagent.radius;
                oVelocity = oagent.GetVelocity();
            }
            else if(neighbor.GetComponent<LeaderAgent>()){
                LeaderAgent oLeader = neighbor.GetComponent<LeaderAgent>();
                oRadius = oLeader.radius;
                oVelocity = oLeader.GetVelocity();
            }
            
            float r_ij = oRadius + radius;
            ...
            float deltaV_ji = Vector3.Dot((oVelocity - GetVelocity()), t_ij);
